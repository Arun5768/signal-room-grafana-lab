...
test('records successful and failed checkouts as observable signals', async (context) => {
  ...
  assert.match(metrics, /signal_room_checkout_requests_total\{outcome="success"\} 1/);
  assert.match(metrics, /signal_room_checkout_requests_total\{outcome="error"\} 1/);

  const { events } = await fetch(`${baseUrl}/api/events`).then((response) => response.json());
...
test('rejects unknown failure scenarios', async (context) => {
  ...
  assert.deepEqual(await response.json(), { error: 'scenario must be normal, slow, or failure' });
});
