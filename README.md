...
The repository also contains a public Cloudflare Worker version of the failure injector. It provides a live interaction surface and structured Workers logs; the complete Prometheus, Loki, Tempo, Alloy, and Grafana investigation workflow remains the Docker deployment below.
...
- A Cloudflare Worker deployment with an exportable per-session evidence trail.

## Public demo

**Live:** <https://signal-room-grafana-lab.arunchandel1780.workers.dev>

Deploy the live interaction surface with:

```bash
wrangler deploy
```

Workers Observability is enabled in `wrangler.jsonc`, so each synthetic checkout emits a searchable structured event. Visitors can also download their session trail as JSON. The public demo intentionally does not claim to host Grafana itself: Grafana, Loki, Tempo, Prometheus, and Alloy run through the complete local stack.
