...
export function createSignalRoom({
  ...
  async function handleCheckout(request, response) {
    ...
    if (!allowedScenarios.has(scenario)) {
      return sendJson(response, 400, { error: 'scenario must be normal, slow, or failure' });
    }
    ...
    try {
      const delay = scenario === 'slow' ? randomBetween(1100, 1600) : randomBetween(50, 180);
      ...
      metrics.observe({ outcome, scenario, durationSeconds: durationMs / 1000 });
      const event = {
      ...
      return sendJson(
        response,
        200,
        { orderId: randomUUID(), outcome, scenario, durationMs, traceId: context.traceId },
        context.traceId
      );
    } catch (error) {
      failure = error;
      ...
      return sendJson(
        ...
        { error: error.message, outcome, scenario, durationMs, traceId: context.traceId },
        context.traceId
      );
    }
  }
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      logger.error('request failed before completion', { error: error.message });
