...
export function createMetrics({ buckets = DEFAULT_BUCKETS } = {}) {
  ...
  function render() {
    ...
    for (const outcome of ["success", "error"]) {
      lines.push(
        `signal_room_checkout_requests_total{outcome="${outcome}"} ${requestCounts.get(outcome) ?? 0}`,
      );
    }
    ...
    buckets.forEach((bucket, index) => {
      lines.push(
        `signal_room_checkout_duration_seconds_bucket{le="${bucket}"} ${histogramCounts[index]}`,
      );
    });
    lines.push(
      `signal_room_checkout_duration_seconds_bucket{le="+Inf"} ${histogramCount}`,
    );
