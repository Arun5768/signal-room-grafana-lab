...
for (const scenario of ['normal', 'slow', 'failure']) {
  ...
  if (response.status !== expectedStatus || !payload.traceId) {
    throw new Error(`Smoke test failed for ${scenario}: ${response.status} ${JSON.stringify(payload)}`);
  }
  process.stdout.write(`✓ ${scenario}: ${response.status}, trace ${payload.traceId}\n`);
}
